...
- 각 카드에 등록자 표시("내가 등록"/"등록자: OOO")를 추가하고, 이유 문구도 남의 것일 때는 "OOO님이 다녀왔고..." 식으로 등록자를 명시
...
**예외 — 타 사용자 공유**: 기획서는 "본인 목록만 조회"였지만, 이후 사용자 요청으로 목록/상세 화면에서 모든 사용자의 맛집을 볼 수 있도록 변경했습니다. 등록·수정·삭제는 여전히 본인 것만 가능합니다 (읽기는 공개, 쓰기는 비공개).
